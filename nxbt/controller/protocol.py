...
class ControllerProtocol():
	...
	def set_motion_inputs(self, inputs):
	
		#print (inputs)
		self.accel_x = inputs[0]
		self.accel_y = inputs[1]
		self.accel_z = inputs[2]
		self.gyro_x = inputs[3]
		self.gyro_y = inputs[4]
		self.gyro_z = inputs[5]
	...
	def set_imu_data(self):
		...
		# generate gyro in -180 to 180 range
		# convert to 16-bit little endian int

		# -90:	0xA6, 0xFF
		# 90:	0x5A, 0x00
		val = int(math.sin(perf_counter() * 3) * 180)
		...
		imu_accel_dec = [-651, -3, 4105]
		imu_gyro_dec = [0, 0, 0]

		# 16-bit int, signed, little endian
		imu_data = []
		for frame in range(3):
			for i in range(3):
				imu_data += imu_accel_dec[i].to_bytes(2, byteorder = 'little', signed = True)
			#for i in range(3):
			imu_data += imu_gyro_dec[0].to_bytes(2, byteorder = 'little', signed = True)
			imu_data += self.gyro_y.to_bytes(2, byteorder = 'little', signed = True)
			imu_data += self.gyro_x.to_bytes(2, byteorder = 'little', signed = True)
		#print(self.gyro_x.to_bytes(2, byteorder = 'little', signed = True))

		#for frame in range(3):

		#	imu_data += self.accel_x.to_bytes(2, byteorder = 'little', signed = True)
		#	imu_data += self.accel_y.to_bytes(2, byteorder = 'little', signed = True)
		#	imu_data += self.accel_z.to_bytes(2, byteorder = 'little', signed = True)
		#	imu_data += self.gyro_x.to_bytes(2, byteorder = 'little', signed = True)
		#	imu_data += self.gyro_y.to_bytes(2, byteorder = 'little', signed = True)
		#	imu_data += self.gyro_z.to_bytes(2, byteorder = 'little', signed = True)
		#for i in range(6):
		#	print (imu_data[i].hex() + " ")
